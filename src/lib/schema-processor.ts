...
import { ContentType, OpenAPIDefinition, ParamSchema, PropertyOptions, SchemaType } from '../types.js';
...
export class SchemaProcessor {
	...
	resolveTSNodeType(node: any): OpenAPIDefinition {
		...
		if (t.isTSVoidKeyword(node) || t.isTSNullKeyword(node) || t.isTSUndefinedKeyword(node)) return { type: 'null' };
		...
		if (t.isTSUnionType(node)) {
			...
			const nullableTypes = node.types.filter((type: any) => t.isTSNullKeyword(type) || t.isTSUndefinedKeyword(type) || t.isTSVoidKeyword(type));

			const nonNullableTypes = node.types.filter((type: any) => !t.isTSNullKeyword(type) && !t.isTSUndefinedKeyword(type) && !t.isTSVoidKeyword(type));
			...
			return {
				oneOf: node.types
					.filter((type: any) => !t.isTSNullKeyword(type) && !t.isTSUndefinedKeyword(type) && !t.isTSVoidKeyword(type))
	...
	public createRequestParamsSchema(params: OpenAPIDefinition, isPathParam: boolean = false): ParamSchema[] {
	...
	public createRequestBodySchema(body: OpenAPIDefinition, description?: string): any {
		const schema: any = {
			content: {
				'application/json': {
					schema: body,
	...
	public getSchemaContent({ tag, paramsType, pathParamsType, bodyType, responseType }: any): {
		...
		if (this.schemaType === 'zod') {
			const schemasToProcess = [paramsType, pathParamsType, bodyType, responseType].filter(Boolean);
