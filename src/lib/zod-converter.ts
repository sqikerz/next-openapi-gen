...
export class ZodSchemaConverter {
	...
	findRouteFilesInDir(dir: string, routeFiles: string[]) {
		try {
			...
			for (const file of files) {
				...
				} else if (file === 'route.ts' || file === 'route.tsx' || (file.endsWith('.ts') && file.includes('api'))) {
				...
				ExportNamedDeclaration: (path) => {
					if (t.isVariableDeclaration(path.node.declaration)) {
						path.node.declaration.declarations.forEach((declaration) => {
							if (t.isIdentifier(declaration.id) && declaration.id.name === schemaName && declaration.init) {
						...
						if (t.isIdentifier(path.node.declaration.id) && path.node.declaration.id.name === schemaName) {
							...
							if (t.isTSTypeReference(typeAnnotation) && t.isIdentifier(typeAnnotation.typeName) && typeAnnotation.typeName.name === 'z.infer') {
								...
								case 'omit':
									if (node.arguments.length > 0 && t.isObjectExpression(node.arguments[0])) {
										node.arguments[0].properties.forEach((prop) => {
											if (t.isObjectProperty(prop) && t.isBooleanLiteral(prop.value) && prop.value.value === true) {
				...
				TSTypeAliasDeclaration: (path) => {
					if (t.isIdentifier(path.node.id)) {
						...
						if (
							...
							if (path.node.typeAnnotation.typeParameters && path.node.typeAnnotation.typeParameters.params.length > 0) {
	...
	processZodNode(node: t.Node): OpenApiSchema {
		...
		if (t.isCallExpression(node) && t.isMemberExpression(node.callee) && t.isCallExpression(node.callee.object)) {
			return this.processZodChain(node);
		}
	...
	processZodLazy(node: t.CallExpression): OpenApiSchema {
		...
		if (node.arguments.length > 0 && t.isArrowFunctionExpression(node.arguments[0]) && node.arguments[0].body) {
	...
	processZodDiscriminatedUnion(node: t.CallExpression): OpenApiSchema {
		...
		const schemas = schemasArray.elements.map((element) => this.processZodNode(element)).filter((schema) => schema !== null);
	...
	processZodUnion(node: t.CallExpression): OpenApiSchema {
		...
		if (unionItems.length === 2) {
			const isNullable = unionItems.some((item) => item.type === 'null' || (item.enum && item.enum.length === 1 && item.enum[0] === null));

			if (isNullable) {
				const nonNullItem = unionItems.find((item) => item.type !== 'null' && !(item.enum && item.enum.length === 1 && item.enum[0] === null));
		...
		const allSameType = unionItems.length > 0 && unionItems.every((item) => item.type === unionItems[0].type && item.enum);
	...
	processZodObject(node: t.CallExpression): OpenApiSchema {
		...
		objectExpression.properties.forEach((prop, index) => {
			if (t.isObjectProperty(prop)) {
				...
				if (t.isCallExpression(prop.value) && t.isMemberExpression(prop.value.callee) && t.isIdentifier(prop.value.callee.object)) {
					...
					if (t.isStringLiteral(node.arguments[0])) {
						...
						node.arguments[0].properties.forEach((prop) => {
							if (
								...
								(t.isStringLiteral(prop.value) || t.isNumericLiteral(prop.value) || t.isBooleanLiteral(prop.value))
							) {
	...
	isOptional(node: t.CallExpression) {
		...
		if (t.isCallExpression(node) && t.isMemberExpression(node.callee) && t.isIdentifier(node.callee.property) && node.callee.property.name === 'optional') {
			return true;
		}
	...
	hasOptionalMethod(node: t.CallExpression) {
		...
		if (
			t.isMemberExpression(node.callee) &&
			t.isIdentifier(node.callee.property) &&
			(node.callee.property.name === 'optional' || node.callee.property.name === 'nullable' || node.callee.property.name === 'nullish')
		) {
			return true;
		}
			...
			traverse.default(ast, {
				...
				VariableDeclaration: (path) => {
					path.node.declarations.forEach((declaration) => {
						if (t.isIdentifier(declaration.id) && declaration.init) {
							const schemaName = declaration.id.name;
							if (this.isZodSchema(declaration.init) && !this.zodSchemas[schemaName] && !this.processingSchemas.has(schemaName)) {
								console.log(`Pre-processing Zod schema: ${schemaName}`);
								this.processingSchemas.add(schemaName);
								const schema = this.processZodNode(declaration.init);
								if (schema) {
	...
	isZodSchema(node) {
		if (t.isCallExpression(node)) {
			...
			if (t.isMemberExpression(node.callee) && t.isIdentifier(node.callee.object) && node.callee.object.name === 'z') {
				return true;
			}
