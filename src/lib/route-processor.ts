...
export class RouteProcessor {
						...
						if (t.isVariableDeclarator(decl) && t.isIdentifier(decl.id)) {
							if (this.isRoute(decl.id.name)) {
								...
								if (!this.config.includeOpenApiRoutes || (this.config.includeOpenApiRoutes && dataTypes.isOpenApi)) {
									...
									if (pathParams.length > 0 && !dataTypes.pathParamsType) {
										console.warn(
											`Route ${routePath} contains path parameters ${pathParams.join(', ')} but no @pathParams type is defined.`,
										);
									}
	...
	private addRouteToPaths(varName: string, filePath: string, dataTypes: DataTypes): void {
		...
		// Add responses
		definition.responses = responses ? this.schemaProcessor.createResponseSchema(responses, responseDescription) : {};
